...
* **NODE_PATH** - Where node.js is installed on your dev machine that is running JPack.  On OS X or Linux you're likely to find this in `~/.bash_profile`  Required only if you want to create critical path CSS.
* **PATH** - This is the PATH environment variable that JPack temporarily creates so it can run the critical CLI program.  If you're having trouble getting critical CSS inlining to work make sure to check this and add any paths that Node.js would require to run.  This path must also include the containing folder for pathToCriticalBinary.  Required only if you want to create critical path CSS.
